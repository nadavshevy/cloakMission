...
class IsolatedHouse:
    ...
    def __init__(self, working_directory, protected_files):
        ...
        self.username = ''.join(random.choice(string.ascii_uppercase) for i in range(12))
    ...
    def start(self, shell_command):
        ...
        print("Starting process with user %s" % self.username)
        print("Working directory %s" % self.working_directory)
        os.system("/usr/sbin/useradd -r %s -g bots" % self.username)

        for fname in self.protected_files:
            print("Protecting file %s" % fname)
            os.chown(fname, pwd.getpwnam(self.username).pw_uid, -1)
            os.chmod(fname, stat.S_IRUSR | stat.S_IRWXU)

        working_directory = self.working_directory
        self.child_queue = Queue()
        shell_command = "unshare -n -- su %s -c \"(umask 077 && %s)\"" % (self.username, shell_command)
        shell_command = shlex.split(shell_command.replace('\\','/'))
    ...
    def kill(self):
        ...
        if self.is_alive:
            ...
            self.child_queue.put(None)
            os.system("pkill -9 -u %s" % self.username)
            print("Removing user %s" % self.username)
            os.system("/usr/sbin/userdel %s" % self.username)
...
def get_sandbox(working_dir, secure=None, protected_files=None):
    if secure is None:
        secure = _SECURE_DEFAULT
    if secure:
        return IsolatedHouse(working_dir, protected_files)
    else:
