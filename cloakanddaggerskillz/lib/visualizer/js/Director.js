...
function Director(vis) {
	...
	this.lastTurnStopped = undefined;
...
Director.prototype.loop = function(delay) {
    ...
    //slowmo
    if (this.vis.state.options.slowmo_enabled) {
        if (Math.floor(this.time) + 2 <= Math.floor(this.duration) && this.vis.piratesMap.attackAboutToHappen()) {
            if (Math.floor(this.defaultSpeed / 4) < 1) {
                this.speed = 1;
            }
            else {
                this.speed = Math.floor(this.defaultSpeed / 4);
            }
        }
        else {
            this.speed = this.defaultSpeed;
