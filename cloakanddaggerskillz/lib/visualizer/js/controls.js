...
//                    $scope.turn = $scope.visualizer.state.time | 0;
                    ...
                    // Info for/from menu buttons
                    var showRange = $scope.visualizer.state.config['showRange'];
                    var label = $scope.visualizer.state.config['label'];
                    var imgPrefix = "background-image:url(../visualizer/data/img/";
                    var imgSuffix = ".png)";

                    $scope.showRange = showRange ? 'on' : 'off' ;
                    $scope.label = ['off', 'team', 'game-id'][label];

                    var showRangeImg = showRange ? 'kidnap' : 'speed_powerup';
                    var labelImg = ['Crown', 'Diamond', 'GoldenBlock'][label];

                    $scope.showRangeImg = imgPrefix + showRangeImg + imgSuffix;
                    $scope.labelImg = imgPrefix + labelImg + imgSuffix;
...
//        $scope.visualizer.director.slowmoTo(stop / 2);
    ...
    $scope.toggleAttackRadius = function() {
        var showRange = !($scope.visualizer.state.config['showRange']);
        $scope.visualizer.state.config['showRange'] = showRange;
        $scope.visualizer.director.draw();
    };
