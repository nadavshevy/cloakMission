...
def do_turn(game):
    ...
    game.debug("(max) cloak reload turns: " + str(game.cloak_reload_turns))
    game.debug("(max) cloak duration: " + str(game.cloak_duration))
    game.debug("my turns to cloak: " + str(game.my_turns_to_cloak))
    game.debug("my cloak turns of pirate 0: " + str(game.my_living_pirates()[0].cloak_turns))
    game.debug("enemy turns to cloak: " + str(game.enemy_turns_to_cloak))
    game.debug("enemy cloak turns of pirate 0: " + str(game.enemy_living_pirates()[0].cloak_turns))
...
def try_cloak(game, pirate):
    if game.my_turns_to_cloak == 0:
        game.cloak(pirate)
        return True
    return False
